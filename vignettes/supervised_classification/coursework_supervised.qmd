...
From this chart we can see that the main differentiator between the two groups are from environmental impact, energy efficiency and energy consumption. The other variables don't show a significant difference in the central tendencies of the two classes.
...
From the violin plots we are able to see that the central tendencies of the median are representative of the high densities in the data. Again the three main differences in the pre and post 30s houses are in environmental impact, energy efficiency and energy consumption.

## Preprocessing

Before fitting a LDA or QDA classifier it is important to check that the data follows the assumptions of the model to maximize and bring validity to the results we get.
...
Energy efficiency, environmental impact and energy consumption seem to roughly follow normal distribution while the others are harder to evaluate. In this case I will be checking the 4 other features using a QQ plot which will clearly show us if the normality assumption is violated.
...
One method of normalizing the features is to use the yeo-johnson transformation which is a statistical technique used to transform non-normal distributed data into more normal. The reason for using the yeo-johnson method is that it deals well with both negative and non-negative values.
...
Quadratic discriminant analysis is another method worth checking as QDA allows us to model non-linear class separations, while LDA checks for linear separators.
...
To check the LDA model for homoscedasticity we can use PCA to plot a 2d visualisation of the clusters/classes which we can check for similarity in their spread and shape, this would prove the assumption and give validity to our 
model.

After visualising the plot, I discovered that there was an outlier/ incorrectly input row of data in the test set (row: 7895) which had -64 for energy consumption, while this might be correct (might have solar panels and sell energy back to the grid) I will be omitting this value in the visualisation below.

![](../../data_cache/vignettes/supervised_classification/pca_plot.png){width="100%" height="600px"}

From this plot we are able to make a judgement that the LDA model does follow the assumption of equal covariance matrices with all classes sharing the same/similar covariance structure.
...
Random Forest also gives us a feature importance metric which shows us which features were most important when classifying the data. The model tells us that when it comes to classifying Pre and Post 30s current energy consumption, current heating costs and current energy efficiency are the 3 most important features. All features included in the model are above 0.1 which indicate that they have moderate predictive power.
...
|10/12/2025 | Chat GPT |To help understand why my new code didn't split the data correctly in my preprocessing function | typed in my function to help me understand what was wrong|no link available (used desktop)  | code base  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
